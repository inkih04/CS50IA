...
def joint_probability(people, one_gene, two_genes, have_trait):
    ...
    for person in people:
        ...
        if people[person]["father"] == None and people[person]["mother"] == None:
            person_probability = PROBS["gene"][gene]
        else:
            father_probability, mother_probability = get_parentsProbability(one_gene, two_genes, people, person)#returns the probability of passing a gene
            if gene == 1: #Son gets one gene
                person_probability = father_probability*(1-mother_probability) + mother_probability*(1-father_probability)

            elif gene == 2: #Son gets two genes
                person_probability = father_probability*mother_probability

            else:#Son doesn't get any gen 
                person_probability = (1-father_probability)*(1-mother_probability)

        if person in have_trait:
            person_probability = person_probability*PROBS["trait"][gene][True]
        else:
            person_probability = person_probability*PROBS["trait"][gene][False]

        probability = probability*person_probability
    return probability


def get_parentsProbability(one_gene, two_genes, people, person):
    mother_probability = 0
    father_probbability = 0
    ...
    else: #Doesn't have the gene
        mother_probability = PROBS["mutation"]
        
    #father

    if  father in one_gene:
        father_probbability = 0.5 

    elif father in two_genes:
        father_probbability = 1 - PROBS["mutation"]

    else: #Doesn't have the gene
        father_probbability = PROBS["mutation"]
    
    return father_probbability, mother_probability
